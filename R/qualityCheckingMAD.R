...
mad_filter<-function(metric, nMad=5, direction="both") {
  ...
  ### allowded number of mads, and a string indicatin direction of the deviation to detect.
...
groups<-NULL 
if(args[8] !="Null")  groups<-args[8]

main(args[1], args[2], args[3],  
     detection=as.numeric(args[4]),
     exp_option = args[5],
     nMad = as.numeric(args[6]),
     direction = args[7], groups = groups,
     propmt_threshold = as.numeric(args[9]), 
     propsp_threshold = as.numeric(args[10]), 
     nb_filters = as.numeric(args[11]),
